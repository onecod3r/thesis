# 2026-07-15 — Motion-Energy Test Report & 1st-Place Landmark Cross-Check
...
1. Results of the GISLR landmark motion-over-time ("motion-energy") test
   ...
   evaluation, and a keep/discard landmark recommendation.
2. A follow-up experiment run for this report: how much of the measured motion
   is z-axis noise (xy vs xyz decomposition).
3. Cross-check against the Kaggle GISLR **1st-place solution**
   (`src/gislr.0.competition.entry.1st.ipynb`): which landmarks it uses, whether
   it uses the whole dataset, and how its subset compares with ours.
4. The **GRU training in detail** (§6): why the GRU was chosen over an LSTM,
   the full training protocol, both runs — the full-543 baseline and the
   **ME-126** reduced-landmark run — with accuracy, per-class accuracy, and a
   class-level comparison of what the landmark reduction changed.
...
Holistic landmarks actually carry signal?"* before we spend GPU-hours on models
that ingest all of them. The motion-energy test is the first, cheapest,
...
- **Hands:** fingertips move most (index tip 8 > middle tip 12 > pinky tip 20),
  wrist and thumb base least (0, 1, 2) — exactly the articulation hierarchy you'd
  expect if the signal is real. Right hand ≈ left hand in magnitude, but the
...
- The commented-out `POSE` list maps to pose landmarks **11–16 + 23–24**
  (shoulders, elbows, wrists, hips) — *exactly* the upper-body subset our
...
## 6. GRU training in detail — full-543 baseline vs ME-126
...
| | baseline `20260713-213000` | ME-126 `20260715-190729` |
|---|---|---|
...
`src/models/gislr/gru/20260715-190729/cache/per_class_vs_baseline.csv`):
...
Run folders: `src/models/gislr/gru/20260713-213000/` and
`src/models/gislr/gru/20260715-190729/` (each: README.md + data.md + assets +
per-class CSV); leaderboard: `src/models/README.md`.

## 7. Follow-ups filed in TODO.md

- Global xy-only re-aggregation of the motion-energy summaries (z-noise
  correction at full-dataset scale).
- Landmark-subset training ablations: exact 1st-place 118 (no pose), xy-only
  channels, lag-1/lag-2 motion features.
- Within-class consistency + cross-class discriminability (ANOVA-style) — the
  instrument that can rank the face landmarks motion energy can't.
