...
def _run_epoch(model, loader, criterion, device, bar, phase,
               optimizer=None, scaler=None, grad_clip=5.0):
...
def _build_meta(*, run_dir, dataset, arch, subset, coords, feature_dim, n_params,
                n_classes, hyp, regime, source, history, best_val_acc,
                epochs_done, early_stopped, finished, wall_time_min, notes):
    spec = ARCHS[arch]
    ...
    return {
        ...
        "split": {"strategy": "stratified 90/10", "random_state": D.SEED,
                  "n_val": D.N_VAL},
        "training": {"regime": regime, "source": source,
                     "epoch_cap": hyp["epochs"], "epochs_trained": epochs_done,
                     "best_epoch": best_epoch, "early_stopped": early_stopped,
                     "finished": finished,
                     "wall_time_min": round(wall_time_min, 1)},
        "hyperparameters": {**hyp, "seed": D.SEED, "max_seq_len": D.MAX_SEQ_LEN,
                            "num_workers": 0, "loss": "CE + label smoothing 0.1",
                            "precision": "AMP"},
        "metrics": {"train_val_acc": round(float(best_val_acc), 4),
                    "eval_status": "pending", "overall_accuracy": None,
                    "macro_accuracy": None, "median_class_accuracy": None,
...
def train_run(*, arch: str, subset_name: str, hyp: dict, regime: str,
              source: str, coords: str = "xyz", dataset: str = "gislr",
              data_dir: Path | None = None, notes: str = "") -> Path:
    ...
    tr_data, tr_off = D.build_subset_cache(train_split, "train", subset, coords, data_dir)
    ...
    meta_kw = dict(run_dir=run_dir, dataset=dataset, arch=arch, subset=subset,
                   coords=coords, feature_dim=feature_dim, n_params=n_params,
                   n_classes=len(sign2idx), hyp=hyp, regime=regime, source=source,
                   notes=notes)
    ...
    for epoch in range(start_epoch, hyp["epochs"]):
        tr_loss, tr_acc = _run_epoch(model, train_loader, criterion, device, bar,
                                     f"ep{epoch + 1} train", optimizer, scaler,
                                     hyp["grad_clip"])
        val_loss, val_acc = _run_epoch(model, val_loader, criterion, device, bar,
                                       f"ep{epoch + 1} val")
        ...
        state = {"epoch": epoch, "model_state": model.state_dict(),
                 ...
                 "sign2idx": sign2idx,
                 "hyp": {**hyp, "seed": D.SEED, "max_seq_len": D.MAX_SEQ_LEN,
                         "num_workers": 0},
                 "feature_dim": feature_dim, "landmarks": subset.array.tolist(),
                 "subset_name": subset_name, "coords": coords,
                 "arch": arch, "training_regime": regime,
                 "epochs_since_gain": epochs_since_gain, "finished": finished,
        ...
        R.write_meta(run_dir, _build_meta(
            **meta_kw, history=history, best_val_acc=best_val_acc,
            epochs_done=epoch + 1, early_stopped=early_stop, finished=finished,
            wall_time_min=wall_now))
        ...
        bar.update(1)
        if early_stop:
            bar.write(f"{tag}: EARLY STOP at epoch {epoch + 1} — no val-acc gain "
