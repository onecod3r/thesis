...
def save_learning_curves(run_dir: Path, title: str | None = None):
    ...
    axes[0].set_title(f"{title or run_dir.name} — loss"); axes[0].legend()
    axes[1].plot(h["train_acc"], label="train")
    axes[1].plot(h["val_acc"], label="val")
    axes[1].set_title(f"{title or run_dir.name} — accuracy"); axes[1].legend()
...
def comparison_row(run_dir: Path) -> dict:
    ...
    return dict(run_id=meta["run_id"], subset=meta["subset"], coords=meta["coords"],
                n_landmarks=meta["n_landmarks"], n_params=meta["n_params"],
                epochs=t["epochs_trained"], early_stopped=t["early_stopped"],
                train_val_acc=m["train_val_acc"], eval_status=m["eval_status"],
